...
| v3.6 | Top 5 tab removed (nav: wall · + · trends). First-time *Glisse vers le haut* hint on the reels, TikTok-style, fades after a few seconds. |
| v3.5 | Tendances as a **TikTok-style feed**: one reel per screen, snap scrolling, autoplay, sound on tap (kept on for the next reels), double-tap ❤️, right rail with likes · **comments** (live bottom sheet) · link · delete own. Only videos are shown — a reel that isn't fetched yet shows *La vidéo arrive…*, never the platform's player. |
| v3.4 | Trends play **inside the app** as real videos: the worker (laptop `npm run worker` or the GitHub Action) fetches each new reel with yt-dlp, converts it, stores it (~2–3 MB) and the card autoplays muted on screen, tap for sound. YouTube and TikTok work as-is; Instagram usually works, and always does with a logged-in session (`YTDLP_BROWSER=brave` on the laptop). Until fetched, the platform's player is the fallback. |
